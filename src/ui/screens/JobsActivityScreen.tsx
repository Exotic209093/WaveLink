...
export function JobsActivityScreen(props: { sf: SfApi; onNavigate: (route: string) => void }): VNode {
  ...
  async function undo(transaction: PushTransaction): Promise<void> {
    ...
    try {
      await props.sf.startDataPush({ orgId: transaction.orgId, objectName: transaction.objectName, operation: 'delete', records: transaction.rollbackIds.map(Id => ({ Id })) });
  ...
  return <div>
    ...
    {recoverable.length > 0 || Object.keys(bulkQueries).length > 0 ? <section class="wl-card" aria-labelledby="recovery-heading" style="margin-bottom:20px"><div class="wl-cardHeader"><div><h2 id="recovery-heading">In-progress & interrupted jobs</h2><div class="wl-muted">Durable export and import checkpoints survive service-worker restarts.</div></div></div><div class="wl-cardSection">{message ? <div class="wl-bannerInfo" role="status">{message}</div> : null}<div class="wl-activityList">{Object.values(bulkQueries).map(checkpoint => <div class="wl-activityItem" key={checkpoint.jobId}><span class="wl-statusDot" data-status="warning" aria-hidden="true" /><div class="wl-activityItem__body"><div class="wl-activityItem__title">BULK QUERY Â· {checkpoint.state}</div><div class="wl-activityItem__sub">org {checkpoint.orgId} Â· Salesforce job {checkpoint.jobId} Â· {checkpoint.soql}</div></div><div class="wl-actions"><button class="wl-buttonNeutral" onClick={() => props.onNavigate('export')}>Resume in Export</button><button class="wl-buttonDestructive" disabled={busyId === checkpoint.jobId} onClick={() => cancelBulkQuery(checkpoint)}>Cancel</button></div></div>)}{recoverable.map(push => <div class="wl-activityItem" key={push.id}><span class="wl-statusDot" data-status={push.status === 'processing' ? 'warning' : 'danger'} aria-hidden="true" /><div class="wl-activityItem__body"><div class="wl-activityItem__title">{push.operation.toUpperCase()} {push.objectName} Â· {push.status}</div><div class="wl-activityItem__sub">{push.processedRecords.toLocaleString()} of {push.totalRecords.toLocaleString()} processed{push.bulkJobId ? ` Â· Salesforce job ${push.bulkJobId}` : ''}{push.lastError ? ` Â· ${push.lastError}` : ''}</div></div><div class="wl-actions">{push.status === 'processing' ? <button class="wl-buttonDestructive" disabled={busyId === push.id} onClick={() => cancelPush(push)}>Cancel</button> : push.resumeSupported ? <button class="wl-buttonNeutral" disabled={busyId === push.id} onClick={() => resumePush(push)}>Resume</button> : <button class="wl-buttonNeutral" onClick={() => props.onNavigate('import')}>Re-open source</button>}</div></div>)}</div></div></section> : null}

    {transactions.length > 0 ? <section class="wl-card" aria-labelledby="undo-heading" style="margin-bottom:20px"><div class="wl-cardHeader"><div><h2 id="undo-heading">Undo history</h2><div class="wl-muted">Rollback windows are local and time-limited.</div></div></div><div class="wl-cardSection"><div class="wl-activityList">{transactions.map(transaction => <div class="wl-activityItem" key={transaction.id}><span class="wl-statusDot" data-status="warning" aria-hidden="true" /><div class="wl-activityItem__body"><div class="wl-activityItem__title">Undo {transaction.operation.toUpperCase()} {transaction.objectName}</div><div class="wl-activityItem__sub">{transaction.rollbackIds.length.toLocaleString()} IDs Â· org {transaction.orgId} Â· expires {new Date(transaction.expiresAt).toLocaleString()}</div></div><button class="wl-buttonNeutral" disabled={busyId === transaction.id || isTransactionExpired(transaction)} onClick={() => undo(transaction)}>Undo</button></div>)}</div></div></section> : null}

    <section class="wl-card" aria-labelledby="activity-heading"><div class="wl-cardHeader"><div><h2 id="activity-heading">Activity trail</h2><div class="wl-muted">{activity.length} matching events Â· migration reports retired with the v0.6 bounded Copy decision</div></div></div><div class="wl-cardSection">
